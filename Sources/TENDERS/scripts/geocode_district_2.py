...
tenders_df = pd.read_csv(r'D:\CDL\Bihar Scraper\bihar\flood-data-ecosystem-Bihar\Sources\TENDERS\data\flood_tenders_all.csv')
...
for idx, row in tenders_df.iterrows():
    tender_slug = str(row['Tender Title :']) + ' ' + str(row['Work Description'])
    tender_slug = re.sub(r'[^a-zA-Z0-9 \n\.]', ' ', tender_slug)
    for district in od_districts:
        if re.findall(r'\b%s\b'%district.lower().strip(), tender_slug.lower()):
...
tenders_df.to_csv(r'D:\CDL\Bihar Scraper\bihar\flood-data-ecosystem-Bihar\Sources\TENDERS\data\floodtenders_districtgeotagged.csv',index=False)
