...
# ===== 1. Paths =====
base_path = Path.cwd() / "Sources" / "NASADEM"
geojson_path = Path.cwd() / "Maps" / "br-ids-drr_shapefile" / "Bihar_subdistrict_final_4326.geojson"

Bihar_sub_gdf = gpd.read_file(geojson_path)

# ===== 2. DEM raster =====
dem_raster = rasterio.open(base_path / "data" / "Bihar_NASADEM_DEM_30.tif")
dem_raster_array = dem_raster.read(1)

mean_dicts = rasterstats.zonal_stats(
    Bihar_sub_gdf.to_crs(dem_raster.crs),
    dem_raster_array,
...
# ===== 3. Slope raster =====
slope_raster = rasterio.open(base_path / "data" / "Bihar_NASADEM_SLOPE_30.tif")
slope_raster_array = slope_raster.read(1)

mean_dicts = rasterstats.zonal_stats(
    Bihar_sub_gdf.to_crs(slope_raster.crs),
    slope_raster_array,
...
output_dir = base_path / "data" / "variables" / "elevation"
